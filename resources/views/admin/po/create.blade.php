...
<script>
    ...
    $('#supplier').change(function() {
        ...
        axios.get(url).then(function(response) {
            res = response.data.supplierProducts;
            if (res) {
                var i = 1;
                if (res) {
                    let tr = '';
                    res.forEach((e) => {
                        ...
                        let image = imgPath('product',e.image);
                        tr += `<tr>
                    <td>
                      ${i++}
                    <input type="checkbox" class="checkbox d-none" name="item_details[${i}][id]" value="${e.pro_id}">
                    </td>
                    <td><img src="${image}" class="img">
                    <input type="hidden" name="item_details[${i}][image]" value="${image}"></td>
                    <td>${e.title}
                    <input type="hidden" name="item_details[${i}][title]" value="${e.title}"></td>
                    <td>${e.sku}
                    <input type="hidden" name="item_details[${i}][sku]" value="${e.sku}"></td>
                    <td>0</td>
                    <td>0</td>
                    ...
                    <td>${e.unit}
                    <input type="hidden" name="item_details[${i}][unit]" value="${e.unit}"></td>
